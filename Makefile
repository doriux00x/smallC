...
test: $(BIN)
	...
	./$(BIN) -a tests/float.c
	./$(BIN) -t tests/lexer.c
	@set -e; for t in run1 run2 run3 run4 run5 runfptr runstruct runfloat runinit runswitch runtypedef runenum rununion runconst runstatic rungoto runbool runvolatile runregister runpreproc runstdarg runclit; do \
	  echo "== $$t =="; \
	  ./$(BIN) tests/$$t.c; \
